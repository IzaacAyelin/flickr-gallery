...
describe('Gallery', () => {
  ...
  it('deletes images from state', done => {

    const testId = '40167481242';
    const imagesLength = initialImages.length;
    wrapper.setState({
      images: initialImages
    }, () => {
      wrapper.instance().deleteImage(testId)
      expect(wrapper.state().images.length).to.eq(imagesLength - 1);
      wrapper.setState({
        images: wrapper.state().images
      }, () => {
        expect(wrapper.children().children().length).to.eq(imagesLength - 1)
        done();
      });
    });
  });

  it('swaps images', done => {
    wrapper.setState({
      images: initialImages
    }, () => {
      wrapper.instance().rearange(['28420720169', '39489067804']);
      wrapper.setState({
        images: wrapper.state().images
      }, () => {
        expect(wrapper.state().images[0].id).to.be.eq('39489067804');
        expect(wrapper.state().images[1].id).to.eq('28420720169');
        done();
      })
    });
  });

  it('get next page of images', done => {
    const firstPage = [
      { id: '39301783895', owner: '156204685@N03', secret: '475b6645b9', server: '4761', farm: 5 },
      { id: '39301758945', owner: '28549294@N05', secret: 'e6ccd03a1b', server: '4719', farm: 5 }
    ]
    ...
    moxios.wait(() => {
      ...
      request.respondWith({
        ...
        expect(wrapper.state().images.length).to.be.equal(firstPage.length+secondPage.length);
        done();
        moxios.uninstall();
      }).catch(done);
    });
    ...
    wrapper.setState({
      ...
      wrapper.instance().getImages({ tags: 'test', page: 2 })
    })
